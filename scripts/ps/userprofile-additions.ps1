...
function Delete-Connector {
  <#
  .SYNOPSIS
    Delete connector — stopping connector, delete offsets, delete connector
  .PARAMETER debeziumUrl
    URL for debezium API; '***' for DEV env
  .PARAMETER connectorName
    connector name, witch offset must be cleaned
  ...
  [void](Invoke-WebRequest "$debeziumUrl/connectors/$connectorName/offsets" -Method DELETE -UseBasicParsing)
  Write-Host "Done"
  Write-Host "Deleting connector..." -NoNewline
  [void](Invoke-WebRequest "$debeziumUrl/connectors/$connectorName/" -Method DELETE -UseBasicParsing)
  Write-Host "Done"
}
